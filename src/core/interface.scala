...
object Subtype:
  ...
  def apply[Tc[_], T, S](info: TypeInfo, idx: Int, anns: Array[Any], tpeAnns: Array[Any], tc: CallByNeed[Tc[S]],
...
object Param:
  ...
  /** constructs a new [[Param]] instance
    ...
    * This method is intended to be called only from code generated by the Magnolia macro, and
    * should not be called directly from users' code. */
  def apply[Tc[_], T, P](name: String, idx: Int, isRepeated: Boolean, typeclassParam: CallByNeed[Tc[P]],
      defaultVal: CallByNeed[Option[P]], annotationsArrayParam: Array[Any],
      typeAnnotationsArrayParam: Array[Any]): Param[Tc, T] = new Param[Tc, T]:
...
abstract class ReadOnlyCaseClass[Typeclass[_], Type](
  ...
  override def toString: String = s"ReadOnlyCaseClass(${typeInfo.full}, ${params.mkString(",")})"

  /** a sequence of [[ReadOnlyParam]] objects representing all of the parameters in the case class
   *
   *  For efficiency, this sequence is implemented by an `Array`, but upcast to a
   *  [[scala.collection.Seq]] to hide the mutable collection API. */
  def params: Seq[ReadOnlyParam[Typeclass, Type]] = parametersArray.toSeq
...
/** [[CaseClass]] contains all information that exists in a [[ReadOnlyCaseClass]], as well as methods and
 *  context required for construct an instance of this case class/object (e.g. default values for constructor
 *  parameters)
 *
 *  @param typeInfo         the name of the case class
 *  @param isObject         true only if this represents a case object rather than a case class
...
abstract class CaseClass[Typeclass[_], Type] (
  ...
  annotationsArray: Array[Any],
  typeAnnotationsArray: Array[Any]
) extends ReadOnlyCaseClass[Typeclass, Type](typeInfo, isObject, isValueClass,
  parametersArray.asInstanceOf[Array[ReadOnlyParam[Typeclass, Type]]], annotationsArray, typeAnnotationsArray):

  /** a sequence of [[Param]] objects representing all of the parameters in the case class
   *
   *  For efficiency, this sequence is implemented by an `Array`, but upcast to a
   *  [[scala.collection.Seq]] to hide the mutable collection API. */
  override def params: Seq[Param[Typeclass, Type]] = parametersArray.toSeq
  ...
  def constructMonadic[Monad[_]: Monadic, PType: ClassTag]
                      (makeParam: Param[Typeclass, Type] => Monad[PType]): Monad[Type]
...
final class SealedTrait[Typeclass[_], Type]
                       (val typeInfo: TypeInfo,
                        ...
                        annotationsArray: Array[Any],
                        typeAnnotationsArray: Array[Any]) extends Serializable:
  ...
  def split[Return](value: Type)(handle: Subtype[Typeclass, Type] => Return): Return =
    @tailrec def rec(ix: Int): Return =
      if ix < subtypesArray.length then
