...
object Monadic {
  ...
  extension [F[_], A, B](fv: F[A])(using inst: Functor[F])
    def map(f: A => B): F[B] = inst.map(fv)(f)

  extension [F[_], A, B](fv: F[A])(using inst: Monadic[F])
    def flatMap(f: A => F[B]): F[B] = inst.flatMap(fv)(f)
}