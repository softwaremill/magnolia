...
object NoCombine extends AutoDerivation[NoCombine]:
  ...
  // implicit def gen[T]: NoCombine[T] = macro Magnolia.derived[T]

  def join[T](ctx: CaseClass[magnolia1.examples.NoCombine, T]): NoCombine[T] = instance { value => 
    ctx.typeInfo.short

  }

  override def split[T](ctx: SealedTrait[NoCombine, T]): NoCombine[T] = instance { value =>
