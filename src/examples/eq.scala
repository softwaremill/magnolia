...
object Eq extends Derivation[Eq]:
  ...
  def join[T](ctx: CaseClass[Eq, T]): Eq[T] = new Eq[T]:
    def equal(value1: T, value2: T) = ctx.params.forall { param =>
  ...
  override def split[T](ctx: SealedTrait[Eq, T]): Eq[T] = new Eq[T]:
    def equal(value1: T, value2: T): Boolean = ctx.split(value1) {
      case sub => sub.cast.isDefinedAt(value2) && sub.typeclass.equal(sub.cast(value1), sub.cast(value2))
    }

  given Eq[String] = _ == _
  given Eq[Int] = _ == _

  given eqOption[T: Eq]: Eq[Option[T]] =
    case (Some(v1), Some(v2)) => summon[Eq[T]].equal(v1, v2)
    case (None, None)         => true
    case _                    => false

  given eqIterable[T: Eq, C[x] <: Iterable[x]]: Eq[C[T]] = (v1, v2) =>
